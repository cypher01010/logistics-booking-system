	...
	'columns' => [
		...
		[
			...
			'value' => function($data) {
				if($data->image_signature == '') {
					...
					return '<a href="' . \Yii::$app->getUrlManager()->createUrl(['delivery/crud/signature', 'id' => $data->id]) . '" target="_blank">view</a>';
				}
			},
		],
		...
		[
			...
			'value' => function($data) {
				...
				$statusList = $deliveryObject->statusList();
		...
		[
			'header' => "<a href='" . \Yii::$app->getUrlManager()->createUrl(['delivery/crud/index', 'sort' => $sortDriver]) . "'>Driver Name</a>",
			//'header' => 'Driver Name',
			'format' => 'raw',
			'value' => function($data) {
