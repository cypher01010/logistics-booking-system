...
class CustomerLoginForm extends Model
{
	...
	{
		if (!$this->hasErrors()) {
			if($this->user->type != \app\models\User::USERTYPE_CUSTOMER) {
	...
	{
		if ($this->_user === false) {
			$this->user = $this->user->getRecordByEmailObject(trim($this->email));
	...
	{
		$userRecord = $this->user->getRecordByEmailObject(trim($this->email));
